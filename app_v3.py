    ...
    def start(self):
        ...
        while True:
            ...
            if 'All systems ready.' in output:
                self.status = True
                return self.status
        return logger.info(self.status)

class UbuntuClient:
    def start_docker(self, port, discord_user_nick):
        docker_name = "ctf-challenge"
        logger.info("Called start_docker function from UbuntuCleint class")
        cmd = f'docker exec -d -t {docker_name} /usr/src/app/setup.sh {port} {discord_user_nick}'
        #logger.info(cmd)
        ...
        while True:
            ...
            if output == '' and process.poll() is not None:
                break
            if '[+] Beacon (HTTP): ALIVE' in output:
                logger.info("Result found")
                logger.info("[+] TCP established")
            else:
...
class FindPort:
    ...
    def set_config(self):
        ...
        #config_file = BASE_DIR/caldera-lightweight/conf/default.yml
        ...
        return HTTP_PORT
        #return {'url': f'http://localhost:{HTTP_PORT}'}
...
def run_task():
    ...
    logger.info(caldera_server.preparation())

    if caldera_server.status:
        #ubuntu_client.start_docker(result['HTTP_PORT'])
        ubuntu_client.start_docker(result, "maouque")
        logger.info(f'Port number is:{result}')
        return result
    else:
        return logger.info({'error': 'Failed to start Caldera'})
